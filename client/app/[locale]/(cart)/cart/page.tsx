    ...
    return (
        <div className="container grid items-start justify-start w-full h-screen grid-cols-1 gap-2 mt-20 md:grid-cols-5">
                            ...
                            <tr key={index} className="border-b border-b-neutral-200">
                                ...
                                <td>
                                    <div className="flex items-center justify-start gap-2">
                                        <Button type="button" onClick={() => handleQuantityCart(item.product, 'reduce')}>-</Button>
                                        <p>{item.quantity}</p>
                                        <Button type="button" onClick={() => handleQuantityCart(item.product, 'add')}>+</Button>
                                    </div>
                                </td>
                                <td>
            ...
            <div className="w-full h-full col-span-1">
                <Card>
                    <CardTitle className="p-4">Subtotal: {formatCurrencyVN(cartLocalStorage.reduce((total, item) => total + Number(item.product.price) * item.quantity, 0))}</CardTitle>
                    <Separator className="w-full" />
                    <CardContent className="flex flex-col items-center justify-start w-full gap-3 px-4 py-6">
                        {cartLocalStorage.map((item, index) => {
                            return (
                                <div key={index} className="w-full flex justify-start items-start gap-3">
