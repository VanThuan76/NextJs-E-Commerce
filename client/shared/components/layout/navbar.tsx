...
export const Navbar = () => {
    ...
    const checkSpecialPage = pathName.split("/")[2] === "product" || pathName.split("/")[2] === "cart" || pathName.split("/")[2] === "my-order"
    ...
    return (
        ...
        >
            ...
            <div className="items-center justify-center hidden lg:flex gap-4">
                ...
                <Popover>
                    <PopoverTrigger className="relative" onClick={() => cartLocalStorage.length > 0 && router.push('/cart')}>
                        <ShoppingCart className="size-5" />
                        <Badge className="absolute -top-2 -right-2 m-0 px-[6px] py-[2px] text-[8px]">
                            {cartLocalStorage.reduce((total, item) => total + item.quantity, 0)}
                        </Badge>
                    </PopoverTrigger>
                    {cartLocalStorage.length === 0 &&
                        <PopoverContent side="bottom" className="w-full flex flex-col justify-start items-start gap-3">
